...
exports.execute = function(local_port,map,defaultPort){
    ...
    */
    function parse_request(buffer) {
        ...
        if (method == 'CONNECT') {
            ...
            if (arr && arr[1] && arr[2] && arr[3] && arr[4]) return {
                method: arr[1],
                host: arr[2],
                port: arr[3],
                httpVersion: arr[4]
            ...
            if (arr && arr[1] && arr[2] && arr[3]) {
                ...
                if (host) {
                    var _p = host.split(':', 2), _r = getHost(arr[2],map);
